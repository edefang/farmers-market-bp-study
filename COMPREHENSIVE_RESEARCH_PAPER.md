...
**3.5 Functional-form check.** After the EDA scatter plot of adherence vs. systolic change raised the question of whether a straight line was really the right way to describe that relationship, I fit linear and polynomial (degree 2 and 3) regressions against the same data, pooled across all participants and separately within each arm, and compared training R² against 5-fold cross-validated R² to check whether the added curvature was capturing real structure or just fitting noise.

## 4. Data Preparation and Analysis

**4.1 Target distribution and baseline check.** Across all 300 participants, mean systolic change was -3.83 mmHg (std 4.18, range -15.50 to 6.40) and mean diastolic change was -2.53 mmHg (std 2.59, range -11.20 to 5.50). Negative values mean BP dropped from baseline to follow-up, so on average BP fell across the whole sample, guided or not. Before comparing groups on outcome, I checked baseline systolic BP for age-related differences, since an imbalance there would confound any later group comparison.

As Figure 1 shows, baseline systolic BP rises gradually across the five age buckets, from a median around 115 mmHg in the 18-29 range up to around 124 mmHg in the 60-65 range, with the spread (interquartile range) also widening at older ages. That's the expected pattern for a real population sample and not a sign of a sampling problem. A small number of high outliers appear in the 50-59 and 60-65 buckets, along with one unusually low outlier in the 60-65 bucket, consistent with natural variation in a 300-person real-data sample rather than a systematic issue.
...
| Group | Systolic Δ mean (std) | Diastolic Δ mean (std) | Mean adherence (%) |
|---|---:|---:|---:|
| Dietitian | -6.07 (4.21) | -4.08 (2.40) | 64.6 |
| AI Assistant | -4.32 (3.48) | -2.54 (2.34) | 55.2 |
| Control | -1.09 (3.17) | -0.96 (2.03) | 34.8 |
...
**4.4 Confound check.** Adherence was unevenly distributed by group exactly as the study design would produce it: 64.6% mean in Dietitian, 55.2% in AI Assistant, 34.8% in Control. It was not meaningfully confounded by age (ranging narrowly from 46.3% to 55.4% mean adherence across age buckets) or by sex (51.2% female vs. 52.1% male), which cleanly isolates group assignment and adherence as the two variables actually driving the outcome, rather than some hidden demographic split. Across the full sample, 11% of participants had near-zero adherence (10% or less), a built-in noise factor present in every arm by design.
...
- Systolic change: one-way ANOVA F = 48.08, p = 8.14e-19; Kruskal-Wallis H = 74.59, p = 6.37e-17.
- Diastolic change: one-way ANOVA F = 47.41, p = 1.35e-18; Kruskal-Wallis H = 75.97, p = 3.19e-17.
- ANCOVA, systolic (group + adherence_pct + baseline_systolic_bp): group effect p = 4.93e-06, adherence effect p = 1.77e-27, model R² = 0.494.
- ANCOVA, diastolic (group + adherence_pct + baseline_diastolic_bp): group effect p = 5.48e-08, adherence effect p = 3.52e-15, model R² = 0.386.
...
Winner: Random Forest. Held-out 80/20 test split: R² = 0.450, RMSE = 3.60, MAE = 2.99. Figure 6 shows the top 15 features behind that model's predictions, led by `adherence_pct` at an importance of 0.457, more than ten times any other single feature.
...
Winner: Random Forest. Held-out 80/20 test split: R² = 0.237, RMSE = 2.16, MAE = 1.75. As with systolic change, Figure 7 confirms `adherence_pct` (importance 0.340) as the dominant feature, with `guidance_sessions_or_interactions` a distant second (0.069).
...
Top features were consistent across both targets, confirming H3.

**5.3 Functional-form check (adherence vs. systolic change).** Pooled across all 300 participants, the linear fit was y = -0.104x + 1.53 (R² = 0.450, r = -0.670, p = 1.61e-40), shown in Figure 8. Moving to a quadratic fit only raised training R² to 0.460, and a cubic to 0.461, both negligible gains. Cross-validated R² told the real story: 0.409 for linear, 0.415 for quadratic, 0.411 for cubic, essentially flat, meaning the added curvature wasn't capturing real structure.

**Figure 8. Linear regression, all groups (general fit).** Adherence vs. systolic BP change across all 300 participants, with the fitted linear regression line and its equation, R², r, and p-value labeled. The pooled functional-form reference for Figures 9 through 11.
...
| Group | r | Linear R² | Poly² R² | Linear CV R² | Poly² CV R² |
|---|---:|---:|---:|---:|---:|
| Dietitian | -0.70 | 0.494 | 0.497 | 0.382 | 0.375 |
| AI Assistant | -0.50 | 0.249 | 0.249 | 0.130 | 0.124 |
| Control | -0.44 | 0.190 | 0.190 | 0.106 | 0.058 |
...
**Figure 11. Linear regression, Control arm.** Adherence vs. systolic BP change for the Control arm only, with fitted line and stats. The flattest slope and weakest fit of the three, where the quadratic term actively hurt cross-validated performance (0.106 to 0.058), the clearest sign in the whole analysis that curvature was fitting noise rather than signal.

![Figure 11: Linear regression, Control arm](../figures/10_regression_control.png)

In every arm, the polynomial term added nothing to training R² and made cross-validated performance worse. A straight linear regression is the correct model for this relationship, both pooled and within each arm.
...
The AI Assistant arm captures a substantial share of the Dietitian arm's benefit (-4.32 mmHg vs. -6.07 mmHg systolic, visible in Figures 2 and 3) without fully matching it. That's meaningful on its own: it suggests guidance doesn't have to come from a human to move the needle, even if a trained dietitian still gets better results, likely by driving deeper or more consistent adherence (64.6% mean vs. 55.2%).

Adherence is the dominant variable in every analysis lens applied here: strongest correlation (Figure 4), strongest ANCOVA covariate, strongest feature importance in the winning model (Figures 6 and 7), and the clearest linear relationship to outcome (Figures 8 through 11). If there's one practical lever in this whole dataset, it's adherence, and that holds regardless of which arm a participant is in.

Random Forest beating the linear baselines by a real but modest margin suggests there's some nonlinear interaction structure in the data, for instance adherence combining with group membership in a way a straight additive model can't capture, rather than curvature in the adherence-outcome relationship itself, which the functional-form check in Section 5.3 confirmed is linear. That's a useful distinction: the tree model's edge comes from combining variables, not from bending the adherence line.

The weak, scattered correlations on individual produce items are best read as noise rather than signal, unsurprising with roughly 50 features spread across just 300 rows. No specific vegetable should be read as more or less protective based on this dataset.
...
- With 300 rows against roughly 50 features, individual produce-item effects can't be reliably estimated; the weak correlations reported in Section 4.3 should not be read as causal or even reliably real.
...
- Diastolic-change modeling performance was meaningfully weaker than systolic (held-out R² = 0.237 vs. 0.450), so predictions on diastolic change specifically deserve more caution than systolic.
...
**Reproducibility:** `notebooks/bp_farmers_market_analysis.ipynb`; scripted pipeline in `scripts/01_eda.py` through `scripts/06_regression_plots.py`. Full intermediate output in `results/eda_summary.md`, `results/modeling_summary.md`, and `results/model_comparison.csv`. All eleven figures referenced above are in `figures/`.
